...
| 组件 | 功能 | 适用人群 |
|------|------|---------|
| **MCP Server** | 10 个工具让 MCP 兼容的 AI 直接操作 Stata | Claude Desktop、Claude Code、Cursor 用户 |
...
1. 从 [Releases](https://github.com/SexyERIC0723/stata-ai-fusion/releases) 页面下载 `stata-ai-fusion-skill.zip`。
...
### MCP Server -- 10 个 AI 驱动的分析工具

服务器暴露 10 个 MCP 工具，任何 MCP 兼容的 AI 助手都可以调用。
...
| 工具 | 说明 | 示例 |
|------|------|------|
| `run_command` | 执行 Stata 代码并返回输出 | `run_command(code="regress price mpg weight, robust")` |
| `run_do_file` | 运行整个 `.do` 文件 | `run_do_file(path="/path/to/analysis.do")` |
